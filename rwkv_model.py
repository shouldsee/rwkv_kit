...
### [shouldsee] override the searching path for cuda kernels

from rwkv.model import current_path

if os.environ.get('RWKV_CUDA_ON') == '1':
    from torch.utils.cpp_extension import load

    ### import the package for loading kernel
    # from rwkv.model import DISABLE_CUBLAS_GEMM
    try:
        load(
            name=f"wkv_cuda",
...
class RWKV(MyModule):
    ...
    def forward_with_internal(self, tokens, state, full_output=False):
        with torch.no_grad():
            w = self.w
            args = self.args
            ...
            if state == None:
                if self.version == 4:
                    ...
                    for i in range(args.n_layer): # state: 0=att_xx 1=att_kv 2=ffn_xx
                        ...
                        state[i*3+2] = torch.zeros(args.n_embd, dtype=atype, requires_grad=False, device=dev).contiguous()
            t = len(tokens)
            outputs = torch.zeros((args.n_layer, t, args.n_embd), dtype=atype, requires_grad=False, device=dev)
            seq_mode = len(tokens) > 1
            ...
            x = x[-1,:] if (seq_mode and (not full_output)) else x
            x = x.to(dtype=dd.atype, device=dd.device)
            outputs = outputs.to(dtype=dd.atype, device=dd.device)
            x = F.layer_norm(x, (args.n_embd,), weight=w['ln_out.weight'], bias=w['ln_out.bias'])
            if w['head.weight'].dtype != torch.uint8:
                x = x @ w['head.weight']
            else:
